...
def compute_kpi_1d(sat, wv, version, dev=False, stop_analysis_period=None, period_analysed_width=30, df=None,
    ...
    if version == 'test1' or version == 'test4' or version == 'test5':
        df['bias_swh_azc_' + wv] = xarray.DataArray(_swh_azc_s1 - _swh_azc_mod, dims=['fdatedt'],
                                                    coords={'fdatedt': df['fdatedt']})
    elif version == 'test2' or version == 'test9':
        df['bias_swh_azc_' + wv] = \
            xarray.DataArray(
                df['oswXA_hs_ww3spec_firstSARpartition'].values[:, 0] - df['oswXA_hs_ww3spec_firstSARpartition'].values[
                                                                        :, 1],
        ...
        df['bias_swh_azc_' + wv] = xarray.DataArray(
            df['oswXA_wl_ww3spec_firstSARpartition'].values[:, 0] - df['oswXA_wl_ww3spec_firstSARpartition'].values[:,
                                                                    1],
            dims=['fdatedt'], coords={'fdatedt': df['fdatedt']})
    else:
    ...
    if version == 'test1':
        ...
        central_ws = 7
        cond_outlier_ww3_hs = (
                    abs(df['ww3_effective_2Dcutoff_hs']) < 50)  # & (abs(df['ww3_effective_2Dcutoff_hs']) > 1)
        cond_wind = (np.abs(df['ecmwf_wind_speed'] - central_ws) < delta_ws)
        logging.debug('nb pts after wind filter %s %1.1f%%', np.sum(cond_wind.values),
                      100.0 * np.sum(cond_wind.values) / cond_wind.size)
        logging.debug('nb pts after NRCS filter %s %1.1f%%', np.sum(cond_nrcs.values),
                      100.0 * np.sum(cond_nrcs.values) / cond_nrcs.size)

        ocean_acqui_filters = cond_outlier_ww3_hs & cond_wind & polarizationcond & cond_nrcs & (
                    abs(df['oswLat']) < LAT_MAX) \
        ...
        cond_quality = (df['oswQualityFlagPartition1'] == 0) | (df['oswQualityFlagPartition1'] == 1) | (
                    df['oswQualityFlagPartition1'] == 2)  # very good
        cond_outlier_ww3_hs = (abs(df['ww3_effective_2Dcutoff_hs']) < 50)
        logging.debug('cond_quality %s', cond_quality.values.sum())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--version',
                        choices=['test1', 'test2', 'test3', 'test4', 'test5', 'test6', 'test7', 'test8', 'test9'],
                        help='version of the KPI-1d', required=False, default='test1')
